...
# X_train and y_train are now directly PyTorch tensors
n_train = 100000
n_digits = 11
...
for epoch in range(epochs):
    ...
    for i in range(0, n_train, batch_size):
        x_batch = X_train[idx[i:i+batch_size]].to(device)
        y_batch = y_train[idx[i:i+batch_size]].unsqueeze(1).to(device)
        optimizer.zero_grad()
